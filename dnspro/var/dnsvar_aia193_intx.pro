PRO dnsvar_aia193_intx, d, name, snaps, swap, var, units, $
                    var_title=var_title, var_range=var_range, var_log=var_log, $
                    info=info
  IF KEYWORD_SET(info) THEN BEGIN
     message, 'AIA Fe XII / XXIV 193 response integrated along X',/info
     RETURN
  ENDIF ELSE BEGIN
     IF n_params() LT 6 THEN BEGIN
        message,'dnsvar_aia193_intx, d, name, snaps, swap, var, units, ' $
     ...
     temp   = total(var,1)
     FOR i=0,si(1)-1 DO var(i,*,*)=temp
     var=var*x[-1]
     var_title='AIA193'
     IF (units EQ "solar") THEN var_title=var_title+" (DN s!u-1!n pix!u-1!n)"
     var_range=[1d-4,1d-1]
     var_log=1
  ENDELSE
END
