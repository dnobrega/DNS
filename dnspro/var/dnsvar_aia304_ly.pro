PRO dnsvar_aia304_ly, d, name, snaps, swap, var, units, $
                    var_title=var_title, var_range=var_range, var_log=var_log, $
                    info=info
  IF KEYWORD_SET(info) THEN BEGIN
     message, 'AIA Fe XII 304 response',/info
     RETURN
  ENDIF ELSE BEGIN
     IF n_params() LT 6 THEN BEGIN
        message,'dnsvar_aia304_ly, d, name, snaps, swap, var, units, ' $
     ...
     FOR j=0,si(2)-1 DO BEGIN
        var(*,j,*) = nel(*,j,*)*nh(*,j,*)*interpol(aia_tresp.a304.tresp,aia_tresp.a304.logte,alog10(tg(*,j,*)))
     ENDFOR
     var(*,*,wh)=1e-32
     var=reform(var)
