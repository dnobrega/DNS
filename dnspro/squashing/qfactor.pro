...
if (count gt 3) then message, string(n_ss)+" nulls (B=0) on grids! Too many!"
...
; save results 
        if scottFlag then begin
              ;DNSi
              pos = STRPOS(file_sav, 'q_perp3d_')
              if twistFlag then begin
                 save, filename=file_sav, q_perp3d
                 save, filename=odir+"twist3d_"+STRMID(file_sav, pos+STRLEN('q_perp3d_')), twist3d
                 save, /compress, filename=odir+"rboundary3d_"+STRMID(file_sav, pos+STRLEN('q_perp3d_')), rboundary3d
              ;DNSf
              endif else save, compress=compress, filename=file_sav, q3d, rboundary3d, xreg, yreg, zreg, delta, q_perp3d
	endif else begin
