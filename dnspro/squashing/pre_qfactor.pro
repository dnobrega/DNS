PRO PRE_QFACTOR, snap0, snapf, step, zbottom, ztop=ztop, factor=factor, nbridges=nbridges, tol=tol, traceFlag=traceFlag, max_trace_steps=max_trace_steps
...
IF ~keyword_set(max_trace_steps) THEN max_trace_steps=200
...
FOR snap=snap0,snapf,step DO BEGIN
    ...
    qfactor, xup_bx, yup_by, zup_bz, xa=xx, ya=yy, za=zz, $
             xreg=[xx[0],xx[-1]], yreg=[yy[0],yy[-1]],zreg=zreg,$
             factor=factor, fstr="q_perp3d_"+STRTRIM(STRING(snap),2), $
             no_preview=1, nbridges=nbridges, /scottFlag, /twistFlag, $
             traceFlag=traceFlag, traceint=snap, max_trace_step=max_trace_steps
