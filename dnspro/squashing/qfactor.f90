...
if (scottFlag) then 
	call trace_scott(vp, q0, q_perp0, rs, re, rbs, rbe, line_length, twist0, i, j)!, twistFlag)
	q(i,j)=q0
	q_perp(i,j)=q_perp0
else
...
if ((bkey1 .or. bkey11 .or. bkey12) .and.&
	...
	q(i,j)=(nxx*nxx+nxy*nxy+nyx*nyx+nyy*nyy) / (bnr(i,j) * (2.0*delta)**2.0)
 	...
 	call ij2vp(i, j, vp)
	call trace_scott(vp, q0, q_perp0, rs, re, rbs, rbe, line_length, twist0, -1, -1)!, .false.)
	q(i,j)=q0
endif
...
if (scottFlag) then 
	call trace_scott(vp, q0, q_perp0, rs, re, rbs, rbe, line_length, twist0, -1, -1)!, twistFlag)
	q(i,j)=q0
	q_perp(i,j)=q_perp0
else
	call interpolateB(vp, bp)	
