module qfactor_common
	...
	nbridges, q1, q2, q1m1, q2m1, q1tq2, Normal_index
...
subroutine round_weigh(vp, round, weigh)
use trace_common
implicit none
real:: w(0:1,0:2), weigh(0:1,0:1,0:1), vp(0:2)
...
use field_common
implicit none
real:: vp(0:2), bp(0:2), weigh(0:1,0:1,0:1), Bcell(0:2,0:1,0:1,0:1)
integer:: round(0:1,0:2), i
!----------------------------------------------------------------------------
call round_weigh(vp, round, weigh)
Bcell=Bfield(:, round(:,0), round(:,1), round(:,2))
forall(i=0:2) Bp(i)=sum(weigh*Bcell(i, :,:,:))
...
use field_common
implicit none
real:: weigh(0:1,0:1,0:1), vp(0:2), bp(0:2), unit_vec_bp(0:2), CurlBp(0:2), alpha
integer:: round(0:1,0:2), i
!----------------------------------------------------------------------------
call round_weigh(vp, round, weigh)
forall(i=0:2)     Bp(i)=sum(weigh*Bfield(i, round(:,0), round(:,1), round(:,2)))
forall(i=0:2) CurlBp(i)=sum(weigh* curlB(i, round(:,0), round(:,1), round(:,2)))
unit_vec_bp= bp/norm2(bp)
...
use trace_common
implicit none
real:: dt, vp0(0:2), vp1(0:2), vp(0:2), vp_tmp(0:2), rs(0:2), re(0:2), unit_vec_bp(0:2), &
...
do sign_dt=-1,1,2
	if (z0flag) then
		...
		call interpolate_unit_vec_B(vp0, unit_vec_bp)
		
		if ( unit_vec_bp(2)*sign_dt .le. 0) then 
...
use rkf45_common
implicit none
integer:: k, s, twistFlag_int, csFlag_int, RK4flag_int, scottFlag_int, q0flag_int, vflag_int, cflag_int, traceflag_int
...
open(unit=8, file='tail.txt', status='unknown')
...
! the setting of 5 threads achieves the best performance here
CALL OMP_set_num_threads(minval([5,nbridges])) 
...
if (scottFlag) then
	...
	!$OMP PARALLEL DO  PRIVATE(k), schedule(DYNAMIC) 
	do k=0, nzm1
		call grad_unit_vec_B_calculate(k)
	enddo
	!$OMP END PARALLEL DO
endif
!----------------------------------------------------------------------------
