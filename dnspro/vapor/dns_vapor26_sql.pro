...
PRO dns_vapor26, var_3Dlist, isnaps, vdffile, do_vdf=do_vdf, do_data=do_data, from_sav=from_sav
    ...
    IF (KEYWORD_SET(do_data)) THEN BEGIN
       IF (N_ELEMENTS(from_sav) EQ 0) THEN BEGIN
          ...
          ENDFOR
       ENDIF ELSE BEGIN
          temp    = fltarr(nx,ny,nz)
          FOR jj=0,ns-1 DO BEGIN
              name = "sqf"
              RESTORE, from_sav+"_"+STRING(isnaps[jj], FORMAT='(I4.4)')+"_qfactor.sav",/verbose
              temp[*,*,0:(size(slq))[3]-1] = slq
              datfile = "var_"+name+".dat"
              openw,lu,datfile,/get_lun
              spawn, 'ls '+datfile
              writeu,lu,float(temp)
              close,lu
              free_lun,lu
              spawn,'raw2vdf -ts '+STRTRIM(string(isnaps[jj]),2)+' -varname ' +name +' '+vdffile+' '+datfile
              spawn,'rm -f '+datfile
              name = "tw"
              temp[*,*,0:(size(tw))[3]-1] = tw
              datfile = "var_"+name+".dat"
              openw,lu,datfile,/get_lun
              spawn, 'ls '+datfile
              writeu,lu,float(temp)
